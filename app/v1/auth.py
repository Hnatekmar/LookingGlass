...
async def regenerate_access_code(
    ...
    try:
        ...
        logger.error(f"Failed to regenerate access code for user {user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to regenerate access code: {e}",
        ) from e
    except Exception as e:
        logger.error(f"Unexpected error regenerating access code for user {user_id}: {e}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to regenerate access code: {e}",
        ) from e
